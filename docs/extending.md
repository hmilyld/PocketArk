...
> `README.md` 的延伸阅读：目录结构、扩展开发完整步骤、约定速查、架构约定与已知限制。
> 相关文档：[AGENTS.md](../AGENTS.md)（开发约定与陷阱）、
> [plugins/README.md](../plugins/README.md)（插件目录规范）、[design.md](design.md)（UI 规范）。
...
│   ├── frontend/      #     前端：views/ settings/ components/ composables/ lib/ schema.ts setup.ts shared.ts
...
> 完整约定、构建期机制与陷阱见 [AGENTS.md](../AGENTS.md)「自动注册的约定与注意事项」。
...
| 插件目录  | `frontend/`（views/settings/components/composables/lib/schema.ts/setup.ts/shared.ts）+ `backend/`（mod.rs/migrations.rs）；`_` 开头目录不注册                 |
